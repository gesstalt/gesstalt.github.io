...
function renderStudents(list = [], containerId) {
  ...
  list
    ...
    .forEach(student => {
      ...
      // Status badge (e.g., "Thesis Submitted")
      const statusHtml = student.status
        ? badge(student.status, student.status === 'Thesis Submitted' ? 'badge-warning' : 'badge-info')
        : '';
      ...
      col.innerHTML = `
        <div class="well text-center">
          ...
          <div class="caption mt-2">
            <h5>${student.name || ''}</h5>
            ${statusHtml} ${tagsHtml} ${profileHtml}
          </div>
        </div>
      `;
